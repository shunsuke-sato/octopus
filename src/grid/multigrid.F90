...
contains
  ...
  !---------------------------------------------------------------------------------
  subroutine multigrid_mesh_half(space, mesh_in, mesh_out, stencil)
    type(space_t),              intent(in)    :: space
    ...
    call mesh_init_stage_2(mesh_out, space, mesh_out%sb, stencil)

    POP_SUB(multigrid_mesh_half)
  end subroutine multigrid_mesh_half

  !---------------------------------------------------------------------------------
  subroutine multigrid_mesh_double(space, mesh_in, mesh_out, stencil)
    type(space_t),              intent(in)    :: space
    type(mesh_t),       target, intent(in)    :: mesh_in
    type(mesh_t),               intent(inout) :: mesh_out
    type(stencil_t),            intent(in)    :: stencil
