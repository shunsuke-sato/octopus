program main
  ...
  allocate(occ_zero(nst_gs,nkp),eps_gs(nst_gs,nkp))
  call read_occ_kvec
...
contains
  subroutine read_mesh
    ...
    file_name=trim(dir_name)//"restart/gs/mesh"
...
!===================================================================================================
  subroutine read_state_info
    ...
    read(20,*)ctmp,nkp
    close(20)
    write(*,"(A,2x,2I5)")"# nst_gs, nkp=",nst_gs, nkp
...
! ground state
    ...
    do ik=1,nkp
       do ist=1,nst_gs
          read(20,*)occ_gs(ist,ik),ctmp(1),eps_gs(ist,ik),ctmp(2),rtmp(2),ctmp(3) &
               ,kvec(1,ik),ctmp(4),kvec(2,ik),ctmp(5),kvec(3,ik),ctmp(6),kweight(ik)
       end do
    end do
    close(20)
...
!    end do
  ...
  subroutine Fermi_Dirac_distribution
    ...
    tot_elec_num = 0d0
    do ik = 1,nkp
       tot_elec_num = tot_elec_num + sum(occ_gs(:,ik))*kweight(ik)
    end do

    write(*,*)"Total # of electrons =",tot_elec_num

    mu_min = minval(eps_gs)
    mu_max = maxval(eps_gs)
    ...
    do 
       ...
       do ik = 1,nkp
          do ist = 1,nst_gs
             ...
             occ_zero(ist,ik) = 2d0/(exp((eps_gs(ist,ik)-mu)*beta)+1d0)
    ...
    ph_num = 0d0
    do ik = 1,nkp
       ph_num = ph_num + sum(abs(occ_gs(:,ik)-occ_zero(:,ik)))*kweight(ik)
    end do
    ph_num = 0.5d0*ph_num
  ...
  subroutine write_dos
    ...
    real(8) :: dos_zero(0:Nw),dos_gs(0:Nw),dos_full(0:Nw)
    real(8) :: dos_zero_s(0:Nw),dos_gs_s(0:Nw),dos_full_s(0:Nw)
    integer :: ik, ist, iw, iw2
    real(8) :: ff

    eps_max = maxval(eps_gs)
    eps_min = minval(eps_gs)
    ...
    do ik = 1,nkp
       do ist = 1,nst_gs
          iw = aint( (eps_gs(ist,ik)-ww_min)/dw )
          dos_zero(iw) = dos_zero(iw) + occ_zero(ist,ik)*kweight(ik)
          dos_gs(iw)   = dos_gs(iw)   + occ_gs(ist,ik)  *kweight(ik)
          dos_full(iw) = dos_full(iw) + 2d0             *kweight(ik)
    ...
    do iw = 0,Nw
       ...
       do iw2 =0,Nw
          ...
          dos_gs_s(iw2)   = dos_gs_s(iw2)   + dos_gs(iw)*ff 
          dos_full_s(iw2) = dos_full_s(iw2) + dos_full(iw)*ff 

       end do

    end do

    write(*,*)"dos-raw integral",sum(dos_zero)*dw,sum(dos_gs)*dw
    write(*,*)"dos-sme integral",sum(dos_zero_s)*dw,sum(dos_gs_s)*dw
    
    open(20,file='nex_w_dist.out')
    do iw = 0,Nw
       ww = ww_min + dw*iw - eps_Fermi
       write(20,"(999e26.16e3)")ww,dos_gs_s(iw),dos_gs_s(iw)-dos_zero_s(iw),dos_full_s(iw)
